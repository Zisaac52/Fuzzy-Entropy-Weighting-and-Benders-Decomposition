# run_localfirst_experiment.py
# -*- coding: utf-8 -*-
"""
专门用于运行 'Local First' 基准算法并记录结果的脚本。
"""
import csv
import os
import time
...
# --- 实验参数 ---
LOG_FILE = 'local_first_runs_log.csv'
...
def run_single_experiment(num_clients, num_helpers, seed):
    """运行单次 local_first 实验。"""
    print(f"\n--- Running Local First: C={num_clients}, H={num_helpers}, Seed={seed} ---")
    ...
    # 2. 使用 local_first 求解器求解
    final_objective, solution, total_time = solve_local_first(client_params, helper_params, network_params)
    ...
    with open(LOG_FILE, 'a', newline='') as f:
        ...
        # 如果文件是空的，先写入表头
        if f.tell() == 0:
            # This is a bit of a workaround. Ideally setup_logger ensures the header.
            # But if the script is run and fails, the file might exist but be empty.
            # Re-opening in 'a' mode won't add headers.
            # A robust solution would check file existence/size before main loop.
            pass # Header is written in setup_logger
